...
class TestSimpleExample:
    """Simple example tests showing clean pytest usage"""
    
    def test_ble_connection(self, ble_client):
        """Test that BLE connection is established"""
        assert ble_client is not None
        assert ble_client.is_connected
    
    def test_required_services_discovered(self, ble_services):
    ...
    @pytest.mark.asyncio
    async def test_device_info_read(self, ble_client, ble_services, serial_capture):
    ...
    @pytest.mark.asyncio
    async def test_wasm_status_read(self, ble_client, ble_services, serial_capture):
    ...
    @pytest.mark.asyncio 
    async def test_data_service_round_trip(self, ble_client, ble_services, serial_capture):
    ...
    @pytest.mark.slow
    @pytest.mark.serial
    def test_serial_capture_works(self, serial_capture):
