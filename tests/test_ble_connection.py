...
def test_all_services_discovered(ble_services, ble_characteristics):
    """Test that all expected services are discovered"""
    
    for service_uuid in EXPECTED_SERVICES:
        assert service_uuid in ble_services
...
@pytest.mark.asyncio
async def test_concurrent_service_access(ble_client, ble_services, ble_characteristics, serial_capture):
    ...
    with serial_capture:
        ...
        # Read device info
        if MANUFACTURER_NAME_UUID in ble_characteristics:
            char = ble_characteristics[MANUFACTURER_NAME_UUID]
            tasks.append(ble_client.read_gatt_char(char))
        
        # Test data service
        if DATA_UPLOAD_UUID in ble_characteristics and DATA_DOWNLOAD_UUID in ble_characteristics:
